...
EXPORT_FUNC APP_UPDATE_DEF(AppUpdate)
{
	...
	// +==============================+
	{
		...
		if (IsKeyboardKeyPressed(&appIn->keyboard, Key_N, true))
		{
			...
			#if 1
			u32 numRepetitions = GetRandU32Range(&app->random, 1, 10);
			Str8 testStr = PrintInArenaStr(scratch, "%s Notification!", GetDbgLevelStr(level));
			NewStrBuffEx(testBuffer, 2048);
			for (u32 rIndex = 0; rIndex < numRepetitions; rIndex++)
			{
				AppendToStrBuffStr(&testBuffer, testStr);
				AppendToStrBuffChar(&testBuffer, ' ');
			}
			NotifyAt(level, 0, testBuffer.str.chars);
			#else
			NotifyPrintAt(level, 0, "%s Notification \"%.*s\"!", GetDbgLevelStr(level), StrPrint(app->mapBackTexturePath));
			#endif
